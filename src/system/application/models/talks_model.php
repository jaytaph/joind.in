...
class Talks_model extends Model {
		...
		if($tid){
			...
			$sql=sprintf('
				select
					...
					(select get_talk_rating(talks.ID)) as tavg,
					(select 
						cat.cat_title
					from 
	...
	*/
	...
	public function search($term,$start,$end){
		...
		$this->db->select('talks.*, count(talk_comments.ID) as ccount, (select get_talk_rating(talks.ID)) as tavg, events.ID eid, events.event_name');
	    $this->db->from('talks');
